...
app.get('/api/cards/print/:type/:id', async (req, res) => {
  try {
    ...
    const html = '<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>بطاقة ' + typeName + ' - ' + person.name + '</title><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&display=swap" rel="stylesheet"><style>* { margin: 0; padding: 0; box-sizing: border-box; }body { font-family: "Cairo", sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px 20px; min-height: 100vh; display: flex; flex-direction: column; align-items: center; }.card { width: 400px; height: 250px; margin: 20px auto; background: linear-gradient(135deg, #1e40af 0%, #3b82f6 50%, #60a5fa 100%); border-radius: 20px; box-shadow: 0 25px 70px rgba(0,0,0,0.4); position: relative; overflow: hidden; color: white; }.card::before { content: ""; position: absolute; top: -50%; right: -50%; width: 200%; height: 200%; background: radial-gradient(circle, rgba(255,255,255,0.15) 0%, transparent 70%); }.card-header { background: rgba(255,255,255,0.15); padding: 12px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid rgba(255,255,255,0.3); position: relative; z-index: 1; }.school-info { display: flex; flex-direction: column; }.school-logo { font-size: 32px; }.school-name { font-size: 14px; font-weight: 700; }.academic-year { font-size: 11px; opacity: 0.9; font-weight: 600; margin-top: 2px; }.card-type { background: rgba(255,255,255,0.25); padding: 6px 18px; border-radius: 25px; font-size: 13px; font-weight: 700; }.card-body { padding: 20px; display: flex; gap: 20px; position: relative; z-index: 1; }.photo { width: 100px; height: 100px; border-radius: 18px; background: rgba(255,255,255,0.25); display: flex; align-items: center; justify-content: center; font-size: 48px; border: 4px solid rgba(255,255,255,0.4); overflow: hidden; flex-shrink: 0; }.photo img { width: 100%; height: 100%; object-fit: cover; }.info { flex: 1; }.info-row { margin-bottom: 8px; }.info-label { font-size: 10px; opacity: 0.9; margin-bottom: 2px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }.info-value { font-size: 14px; font-weight: 700; }.card-footer { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.3); padding: 10px 20px; display: flex; justify-content: space-between; font-size: 11px; font-weight: 600; }.card-number { font-family: "Courier New", monospace; letter-spacing: 2px; background: rgba(255,255,255,0.2); padding: 3px 10px; border-radius: 8px; }.print-btn { display: block; margin: 30px auto; padding: 16px 50px; background: white; color: #1e40af; border: none; border-radius: 35px; font-size: 17px; font-weight: 800; cursor: pointer; box-shadow: 0 15px 40px rgba(0,0,0,0.3); font-family: "Cairo", sans-serif; }@media print { body { background: white; padding: 0; } .card { box-shadow: none; } .print-btn { display: none; } }</style></head><body><div class="card"><div class="card-header"><div class="school-info"><div class="school-logo">🏫</div><div class="school-name">مدرسة النور النموذجية</div><div class="academic-year">العام الدراسي: ' + academicYear + '</div></div><div class="card-type">' + typeName + '</div></div><div class="card-body"><div class="photo">' + (person.photo ? '<img src="' + person.photo + '" alt="صورة">' : '👤') + '</div><div class="info"><div class="info-row"><div class="info-label">الاسم الكامل</div><div class="info-value">' + person.name + '</div></div>' + (type === 'student' ? '<div class="info-row"><div class="info-label">الصف والشعبة</div><div class="info-value">' + (person.grade || '-') + ' - ' + (person.section || '-') + '</div></div>' : '') + (type === 'teacher' ? '<div class="info-row"><div class="info-label">المادة الدراسية</div><div class="info-value">' + (person.subject || '-') + '</div></div>' : '') + (type === 'employee' ? '<div class="info-row"><div class="info-label">الوظيفة</div><div class="info-value">' + (person.role || '-') + '</div></div>' : '') + '<div class="info-row"><div class="info-label">رقم البطاقة</div><div class="info-value">' + cardNumber + '</div></div></div></div><div class="card-footer"><div>' + dt.dateStr + '</div><div>' + academicYear + '</div></div></div><button class="print-btn" onclick="window.print()">🖨️ طباعة البطاقة</button></body></html>';
    res.send(html);
